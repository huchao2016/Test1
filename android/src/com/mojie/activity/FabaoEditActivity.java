...
public class FabaoEditActivity extends Activity implements OnClickListener {
...
//							startActivity(intent);
							...
							Toast.makeText(FabaoEditActivity.this,"发包失败",Toast.LENGTH_SHORT).show();
						}
...
//		i_uid = Integer.parseInt(intent.getStringExtra("uid"));
		tagsid = intent.getStringExtra("tagsid");
...
//		tvFabaoSave.setOnClickListener(this);
		...
		//========================
		tvFabaoLabel.setText(task_type);
		etFabaoTitle.setText(title);
		tvFabaoBudget.setText(budget);
		etFabaoDate.setText(deliverieddate);
		etFabaoOverview.setText(description);
		fabaoUploadListView = (ListView)findViewById(R.id.fabao_upload_listview);
		uploadFileListAdapter = new UploadFileListAdapter(FabaoEditActivity.this, mArray);
		...
		fabaoUploadListView.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> arg0, View arg1, final int position,
				...
				if (mArray.get(position).get("type").equals("0")){
					myUploadBt = new MyUploadBt(FabaoEditActivity.this, "");
					myUploadBt.show();
...
//					myUploadBt.negativeCancel.setOnClickListener(this);
					myUploadBt.uploadFile.setOnClickListener(new OnClickListener() {
						public void onClick(View arg0) {
							...
							try {
								...
								Toast.makeText(FabaoEditActivity.this,"该手机不支持此功能",Toast.LENGTH_LONG).show();
							}	
						}
					});
		...
		params3.add(new BasicNameValuePair("city", city));
		new Thread(new ConnectPHPToGetJSON(URL_GETPROVINCENAME,getprovinceHandler,params3)).start(); 
...
//		new Thread(new ConnectPHPToGetJSON(URL_GETPROVINCE,provinceHandler,params2)).start(); 
		...
		new Thread(new ConnectPHPToGetJSON(URL_GET_TASK_ATTACH,deHandler,params)).start(); 
		pd = new ProgressDialog(FabaoEditActivity.this);
		pd.setProgressStyle(ProgressDialog.STYLE_SPINNER);
		pd.show();
	}
	
	private String URL_GET_TASK_ATTACH = ConstUtils.BASEURL + "gettaskattach.php";
	protected int deresult;
	...
	private Handler deHandler = new Handler(){  
		...
		public void handleMessage(android.os.Message msg) {
			pd.dismiss();
			if (msg.obj == null){//获取数据失败
				Toast.makeText(FabaoEditActivity.this, "网络连接失败",Toast.LENGTH_LONG).show();
			}else{
				try {
					deresult = ((JSONObject)msg.obj).getInt("result");
					if(deresult == 0){
...
//	private String province;
	private String URL_GETPROVINCENAME = ConstUtils.BASEURL + "getprovince.php";
	protected int getproResult;
	protected int province_index;
	protected int city_index;
//	public ArrayList<HashMap<String, String>>rArray;
//	String[] sourceStrArray;
	private Handler getprovinceHandler = new Handler(){  
		@Override  
		public void handleMessage(android.os.Message msg) {
			if (msg.obj == null){//获取数据失败
				Toast.makeText(FabaoEditActivity.this, "网络连接失败",Toast.LENGTH_LONG).show();
				...
				try {		
					getproResult = ((JSONObject)msg.obj).getInt("result");
					if(getproResult == 0){
						province = ((JSONObject)msg.obj).getString("province");
						province_index = ((JSONObject)msg.obj).getInt("province_index");
						city_index = ((JSONObject)msg.obj).getInt("city_index");
						List<BasicNameValuePair> params2 = new ArrayList<BasicNameValuePair>();
						params2.add(new BasicNameValuePair("type", ""+0));
						new Thread(new ConnectPHPToGetJSON(URL_GETPROVINCE,provinceHandler,params2)).start(); 
					...
					if(proResult == 0){
						...
						spinnerProvince.setOnItemSelectedListener(new OnItemSelectedListener() {
							public void onItemSelected(AdapterView<?> arg0, View arg1,
								...
								String provincetemp = list.get(position);
								List<BasicNameValuePair> params3 = new ArrayList<BasicNameValuePair>();
								params3.add(new BasicNameValuePair("type", ""+1));
								params3.add(new BasicNameValuePair("province", provincetemp));
								if(citylist == null){
									citylist = new ArrayList<String>();
								}else{
									citylist.clear();
								}
								if(!provincetemp.equals(province)){
									city_index = 0;
								}
								new Thread(new ConnectPHPToGetJSON(URL_GETCITY,cityHandler,params3)).start(); 
							}
				        ...
				        ArrayAdapter<String> adapter = new ArrayAdapter<String>(FabaoEditActivity.this,R.layout.spinner_experience, R.id.text1,list);  
				        spinnerProvince.setAdapter(adapter);
				        spinnerProvince.setSelection(province_index);
					}else{
					}
				} catch (JSONException e) {
					// TODO Auto-generated catch block
			...
			if (msg.obj == null){//获取数据失败
				...
				try {		
					...
					if(cityResult == 0){
				        ...
				        ArrayAdapter<String> adapter = new ArrayAdapter<String>(FabaoEditActivity.this,R.layout.spinner_experience, R.id.text1,citylist);  
				        spinnerCity.setAdapter(adapter);
				        spinnerCity.setSelection(city_index);
					}else{
					}
				} catch (JSONException e) {
					// TODO Auto-generated catch block
...
//			Log.v("lishide", "save  parsms>>taskid == "+taskid);
...
//				Log.v("lishide", "item  "+mArray.get(i).get("filename"));
		...
		case R.id.btn_fabao_submit:
			...
			description = etFabaoOverview.getText().toString();
			if("".equals(title)) {
				Toast.makeText(FabaoEditActivity.this,"请输入标题",Toast.LENGTH_LONG).show();
				break;
			}else if("".equals(budget)) {
				Toast.makeText(FabaoEditActivity.this,"请输入预算金额",Toast.LENGTH_LONG).show();
				break;
			}else if("".equals(deliverieddate)) {
				Toast.makeText(FabaoEditActivity.this,"请输入交付日期",Toast.LENGTH_LONG).show();
				break;
			}else if("".equals(description)) {
				Toast.makeText(FabaoEditActivity.this,"请输入任务概述",Toast.LENGTH_LONG).show();
				break;
			}
	...
	private Handler upLoadFileHander = new Handler() {
		public void handleMessage(Message msg) {
			...
			case 0:
				...
				uploadFileListAdapter.notifyDataSetChanged();
				ListViewSetHeightUtil.setListViewHeightBasedOnChildren(fabaoUploadListView);
				Toast.makeText(FabaoEditActivity.this, "文件上传成功",Toast.LENGTH_SHORT).show();
				break;
			case 1:
				Toast.makeText(FabaoEditActivity.this, "文件上传失败",Toast.LENGTH_SHORT).show();
				break;
			default:
				break;
			}
